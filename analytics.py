...
def get_admin_stats():
    ...
    try:
        from models import User, Room, Chat, Message, RoomMember
        ...
        return jsonify({
            ...
            'most_active_users': [
                {'name': name, 'messages': count} for name, count in active_users_list
            ],
            'most_active_rooms': [
                {'name': name, 'messages': count} for name, count in active_rooms_list
            ]
        }), 200
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 500 