from models import db, UserModeUsage, Achievement, Message, Comment, Chat, RoomMember
from datetime import datetime
from flask import flash

def track_mode_usage(user_id, room_id, mode):
...
def check_achievements(user_id, room_id):
...
def check_first_steps(user_id, room_id):
    ...
    if message_count >= 1:
        ...
        flash("🎯 Achievement Unlocked: First Steps! You've sent your first message in this room.", "success")

def check_explorer(user_id, room_id):
    ...
    if percentage_used >= required_percentage:
        ...
        flash(f"🗺️ Achievement Unlocked: Explorer! You've used {unique_modes}/{total_modes} modes ({percentage_used:.1%}) in this room.", "success")

def check_ai_whisperer(user_id, room_id):
    """Check if user has had 10 successful AI conversations in the room."""
    # Check if user has already earned this achievement
    existing = Achievement.query.filter_by(
        user_id=user_id,
        room_id=room_id,
        achievement_type='ai_whisperer'
    ).first()
    ...
    user_messages = Message.query.join(Chat).filter(
        Message.user_id == user_id,
        Chat.room_id == room_id,
        Message.role == 'user'
    ).count()
    ...
    if user_messages >= 10:
        ...
        flash("🤖 Achievement Unlocked: AI Whisperer! You've had 10 successful AI conversations in this room.", "success")

def check_collaborator(user_id, room_id):
    ...
    comment_count = Comment.query.join(Chat).filter(
        Comment.user_id == user_id,
        Chat.room_id == room_id
    ).count()
    
    # Get total number of users in the room
    total_users = RoomMember.query.filter_by(room_id=room_id).count() + 1  # +1 for room owner
    ...
    if comment_count >= required_comments:
        ...
        flash(f"👥 Achievement Unlocked: Collaborator! You've helped {comment_count} students in this room.", "success")

def check_learning_master(user_id, room_id):
    """Check if user has used 74% or more of the available modes in the room."""
    # Check if user has already earned this achievement
    existing = Achievement.query.filter_by(
        user_id=user_id,
        room_id=room_id,
        achievement_type='learning_master'
    ).first()
    ...
    if percentage_used >= required_percentage:
        ...
        flash(f"🎓 Achievement Unlocked: Learning Master! You've used {unique_modes}/{total_modes} modes ({percentage_used:.1%}) in this room.", "success")

def get_user_achievements(user_id, room_id):
...
def get_user_mode_usage(user_id, room_id):
    """Get mode usage statistics for a user in a specific room."""
    return UserModeUsage.query.filter_by(
        user_id=user_id,
        room_id=room_id
    ).all() 