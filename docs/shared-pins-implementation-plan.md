...
| Endpoint | Change |
|----------|--------|
| `POST /chat/<id>/pin` | Accept `shared: bool` (default `false`) |
| `POST /chat/<id>/unpin` | No change (removes any pin) |
| `POST /chat/<id>/pin/<pin_id>/share` | New: Promote personal → shared |
| `POST /chat/<id>/pin/<pin_id>/unshare` | New: Demote shared → personal |

### New Query Endpoints

| Endpoint | Purpose |
|----------|---------|
| `GET /chat/<id>/pins/shared` | List shared pins for chat |
| `GET /room/<id>/pins/shared` | List shared pins across room |

### Access Rules

- **View shared pins**: Any room/chat member
- **Create shared pin**: Any room/chat member
- **Unshare own pin**: Pin owner only
...
- [ ] Personal pin not visible to other users
- [ ] Shared pin visible to all chat members
- [ ] Non-member cannot see shared pins
- [ ] Only pin owner can share/unshare own pin
- [ ] Room owner can remove any shared pin
- [ ] AI context includes shared pins for all users
- [ ] AI context includes personal pins only for owner
- [ ] Deleted message cascades to remove pin
- [ ] User removal from room: shared pins remain with "Former member"
- [ ] Session rollback on query failure

---

## Cache Busting Reminder

When deploying shared pins UI:

```html
<!-- templates/base.html -->
<link href="{{ url_for('static', filename='css/chat-improvements.css') }}?v=2.2" ...>
<script src="{{ url_for('app_static', filename='js/pin-toggle.js') }}?v=1.2" ...>
```
