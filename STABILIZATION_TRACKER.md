...
- **Implementation:** Kept the blanket exemption for the JSON endpoints (content-type-shielded) and added explicit `flask_wtf.csrf.validate_csrf` at the top of `/upload`, reading the token from `X-CSRFToken` / `X-CSRF-Token` header or a `csrf_token` form field, so a cross-site `<form>` (which can't set custom headers or know the token) is rejected with 400. **How legit uploads still pass:** the app sets a JS-readable `csrf_token` cookie on every response (`__init__.py:461`, `generate_csrf()`), and a global `fetch` wrapper (`base.html:621`) injects it as `X-CSRFToken` on every request — so the 6 upload callers (which each do a bare `fetch(POST, formData)`) transparently carry a valid token. **Verified end-to-end:** with the cookie-derived `X-CSRFToken` (production config, `WTF_CSRF_ENABLED=True`) the upload passes CSRF and proceeds ("No file provided"); with no token it's rejected ("CSRF validation failed"). **Ultrareview bug_001** (normal-sev: "breaks all 6 upload callers") was **investigated and cleared as a false positive** — the reviewer missed the `fetch` wrapper and the readable cookie. A real-browser upload smoke before the class is still worthwhile belt-and-suspenders.
...
- **Implementation:** (1) Added `MAX_CONTENT_LENGTH` to the base config (default **16MB**, env-overridable via `MAX_CONTENT_LENGTH_MB`, `or 16`-guarded against an empty env value — ultrareview bug_004) so Werkzeug rejects an oversized body with 413 before buffering it. (2) Added `MAX_EXTRACT_CHARS = 5,000,000` and made the PDF and DOCX extraction loops truncate each unit and stop once they pass it, bounding the **multi-unit** accumulation case (many pages/paragraphs). **Verified:** `app.config['MAX_CONTENT_LENGTH'] == 16MB`; suite green. **Residual (ultrareview bug_005, nit):** this does NOT fully close decompression bombs — a *single* hostile unit is still materialized transiently before the cap sees it (`page.extract_text()` returns a whole page as one string; `python-docx`'s `Document()` parses all of `document.xml` at construction, before the paragraph loop). Fully closing it needs a resource-limited parse (subprocess `RLIMIT_AS`, or checking the zip entry's uncompressed size before `Document()`) — a follow-up, not a class blocker (needs an authenticated room member uploading a crafted file). Can't just lower `MAX_CONTENT_LENGTH` below ~11MB without rejecting legitimate 10MB uploads.
