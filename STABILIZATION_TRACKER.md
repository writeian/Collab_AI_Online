...
- **Issue:** Alembic migrations aren't clean, so on every startup the app runs a large block of manual `CREATE TABLE` / `ALTER TABLE` statements to patch the schema. Fragile; a bad deploy can leave a half-built table. **Confirmed reproducible drift (2026-08-20, local run):** the ORM models expect columns the startup patcher never added — `document.key_document_type` and `document_chunk.search_vector` are **missing from the DB**. Any code path that loads/deletes a `document` row crashes with `sqlite3.OperationalError: no such column: document.key_document_type` (hit while cascade-deleting a room; would also break the Library tool and full-text search). This is model↔schema drift: the hand-patch DDL doesn't cover every model column, so it silently falls behind.
- **Fix:** Get Alembic to `head` reliably, make migrations idempotent, and remove the manual DDL from startup once migrations own the schema. Concretely: add the missing columns via a proper migration (not more startup DDL), then verify a clean `alembic upgrade head` on an empty DB reproduces the full model schema with **zero** post-boot patching needed.
- **Verification target:** the drift scan (model `__table__.columns` vs `PRAGMA table_info`) must come back **empty for every mapped table** — script at `scratchpad/cleanup_and_drift.py`.
- **Implementation:** _pending_
- **Files:** `src/main.py:33–531` (`run_production_migrations` + manual DDL), `src/app/__init__.py:162–253` (document-table DDL), `migrations/`, `alembic.ini`. Drifted tables: `document`, `document_chunk`.
